...
#         response = self.client.patch(f"/api/users/{first_user['id']}/", self.mock_user_update_data, format="json")
#         self.assertEqual(response.status_code, status.HTTP_200_OK)
#         data = json.loads(response.content)
...
#         response = self.client.put(f"/api/users/{first_user['id']}/", self.mock_user_update_data, format="json")
#         self.assertEqual(response.status_code, status.HTTP_200_OK)
#         data = json.loads(response.content)
#         self.assertEqual(data["first_name"], self.mock_user_update_data["first_name"])
#         self.assertEqual(data["last_name"], self.mock_user_update_data["last_name"])
...
#     def setUp(self) -> None:
#         self.client = APIClient()
#         self.exist_user = CustomUser.objects.create_user(
#             first_name="I",
#             last_name="Exist",
#             email="i.exist@gmail.com",
#             password="123123123asd"
#         )
#         refresh = RefreshToken.for_user(self.exist_user)
#         self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {refresh.access_token}")
