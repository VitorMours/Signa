...
class TeatcherSingleView(APIView):
  ...
  @swagger_auto_schema(request_body=TeatcherSerializer, responses = {200: TeatcherSerializer})
  def patch(self, request: Request, uuid: str) -> Response:
    teatcher = TeatcherService.get_teatcher_by_id(uuid)
    if not teatcher:
      return Response({"error": "Teatcher not found"}, status=status.HTTP_404_NOT_FOUND)
